...
def _update_index_matrix(batch_size, index_matrix = None):
    if index_matrix is None or index_matrix.size(0) != batch_size * 2:
        ...
        # same domain, i.e. Source to Source or Target to Target -> 1. / float(batch_size)
        ...
        # different domains -> -1. / float(batch_size) 
...
class MultipleKernelMaximumMeanDiscrepancy(nn.Module):
    ...
    def forward(self, z_s, z_t):
        ...
        # sum of all kernels -> kernel matrix
...
class GaussianKernel(nn.Module):
    ...
    def forward(self, X):
        # distance matrix between samples -> (batch_size*2, batch_size*2)
        l2_distance_square = ((X.unsqueeze(0) - X.unsqueeze(1)) ** 2).sum(2)
        self.sigma_square = self.alpha * torch.mean(l2_distance_square.detach())
        return torch.exp(-l2_distance_square / (2 * self.sigma_square)) # apply Gaussian kernel to return similarity matrix


class DomainAdversarialLoss(nn.Module):

    def __init__(self, domain_discriminator, reduction = 'mean', grl = None, max_iters=1000):
        super(DomainAdversarialLoss, self).__init__()
        self.grl = WarmStartGradientReverseLayer(alpha=1., lo=0., hi=1.,
                            max_iters=max_iters, auto_step=True) if grl is None else grl
