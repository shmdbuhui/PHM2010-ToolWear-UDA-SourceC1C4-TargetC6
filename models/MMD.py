...
class Trainset(InitTrain):
    ...
    def __init__(self, args):
        ...
        # multiple Gaussian kernels for MMD
        kernels = [
            utils.GaussianKernel(alpha=2 ** k) for k in range(-3, 2)
        ]
        self.mmd_loss = utils.MultipleKernelMaximumMeanDiscrepancy(kernels)
    ...
    def train(self):
        ...
        for epoch in range(1, args.max_epoch+1):
            ...
            for _ in tqdm(range(num_iter), ascii=True):
                ...
                # MMD loss (measure distribution difference between two domains)
                loss_mmd = self.mmd_loss(feat_s, feat_t)
            ...
            logging.info(f"Train-MMD Loss: {epoch_loss['MMD']/num_iter:.4f}")
